...
const FloatingWhatsApp = () => {
  ...
  useEffect(() => {
    if (visible) {
      ...
      const hideTimer = setTimeout(() => setTooltip(false), 8000);
      return () => {
      ...
      {visible && (
        <div className="fixed bottom-6 right-6 z-50 flex items-end gap-3">
          <AnimatePresence>
            {tooltip && (
              <motion.div
                ...
                className="px-4 py-2.5 rounded-2xl rounded-br-md bg-card border border-border shadow-ceramic-lg max-w-[200px]"
              >
                <p className="text-sm font-medium text-foreground">Need dental care? 💬</p>
          ...
          <motion.a
            ...
            className="relative h-14 w-14 rounded-full bg-whatsapp text-primary-foreground flex items-center justify-center shadow-lg hover:shadow-xl transition-shadow"
